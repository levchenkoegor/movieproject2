...
tonotopy_dir = 'tonotopy'
fd_file_pattern = 'motion_{subject_id}_enorm.1D'

plots_dir = base_dir / "bids_data/derivatives/group_analysis"
plots_dir.mkdir(parents=True, exist_ok=True)

# Find subjects with tonotopy data
subject_list = sorted([sub_dir.name for sub_dir in (base_dir / "bids_data/derivatives").iterdir() if (sub_dir / tonotopy_dir).exists()])
...
def load_fd(subject_id):
    subject_dir = base_dir / "bids_data/derivatives" / subject_id / tonotopy_dir
...
fd_runs = {"Run 1": [], "Run 2": []}
TRs_per_run = 256  # Fixed number of TRs per run

# Collect FD values for all subjects and split into runs
for subject_id in subject_list:
    fd_values = load_fd(subject_id)

    if fd_values is not None and len(fd_values) >= TRs_per_run * 2:  # Ensure we have at least 2 full runs
        fd_runs["Run 1"].extend(fd_values[:TRs_per_run])      # First 256 TRs
        fd_runs["Run 2"].extend(fd_values[TRs_per_run:TRs_per_run*2])  # Next 256 TRs
    else:
...
if len(fd_all_subjects) > 0:
    ...
    plt.savefig(plots_dir / "tonotopy" / "task-tonotopy_overall_fd_distribution_percentage.png")
    plt.close()
else:
...
positions = np.arange(1, 3)  # Positions for Run 1 and Run 2
run_labels = ["Run 1", "Run 2"]

plt.violinplot([fd_runs["Run 1"], fd_runs["Run 2"]], positions, showextrema=False)
plt.boxplot([fd_runs["Run 1"], fd_runs["Run 2"]], widths=0.1, showfliers=False, patch_artist=True,
...
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(plots_dir / "tonotopy" / "task-tonotopy_fd_by_run.png")
plt.close()
